	...
	public static class Draw
	{
		...
		#region Class
			...
			public static void ClassField (FieldInfo field, Type type, object obj, string name, bool isLeft=false, bool allowSceneObject=false)
			{
				...
				else if (type == typeof(Coord3D))
				{
					Coord3D val = Getter<Coord3D>.GetGetter(field)(obj);
					val = Field(val, name);
					if (Cell.current.valChanged && !UI.current.layout)  field.SetValue(obj, val); 
				}
				...
				{
					AnimationCurve val = Getter<AnimationCurve>.GetGetter(field)(obj);
					using (Cell.Row) Label(name);
				...
				else if (type.IsArray)
				{
					...
					Cell.EmptyLinePx(4);
					using (Cell.LineStd)
						using (new Draw.FoldoutGroup(ref opened, name, isLeft:true))
							if (opened)
							{
								...
								if (Cell.current.valChanged && !UI.current.layout)  field.SetValue(obj, arr);
							}
					Cell.EmptyLinePx(4);
				}
				...
				else if (type.IsClass)// || type.IsValueType)
				{
					...
					using (Cell.LineStd)
						using (new Draw.FoldoutGroup(ref opened, name, isLeft:true))
							if (opened)
							{
								object val =  Getter<object>.GetGetter(field)(obj); //field.GetValue(obj);
								Draw.Class(val);
							}
					Cell.EmptyLinePx(4);
				}
